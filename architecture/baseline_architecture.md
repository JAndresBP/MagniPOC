...
- Add **MoveIt 2 for the reBot Arm** — design in `architecture/adr/005-moveit-rebotarm.md`; **`magni_moveit_config` package created** adapting the `rebotarm_ros2` submodule's vendored SRDF/kinematics/controllers config to MAGNI's actual composed URDF and running controllers, wired into `magni_sim_bringup.launch.py` via a `moveit` arg and `scripts/launch-webots-simulation.sh --moveit`; next: verify the self-collision matrix against the real composed geometry, add a proper `gripper_tcp` frame, and integrate whole-body planning with Nav2 for a moving base.
